...
function AdminDashboard() {
    ...
    const StatCard = ({ title, value, icon, prefix = "" }) => (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-shadow h-full">
            <div className="bg-[#f3effb] w-12 h-12 rounded-full flex items-center justify-center text-[#6E39CB] shrink-0">
                {icon}
            </div>
            <div className="min-w-0 flex-1">
                <p className="text-gray-500 font-bold text-[10px] uppercase tracking-widest mb-0.5 leading-tight">{title}</p>
                <p className="text-lg font-['bebas-neue'] text-[#2D2D2D] tracking-wide break-all leading-none mt-1">
        ...
        <div className="bg-[#F4F5F9] min-h-screen flex flex-row font-['Lato']">
            ...
            <div className="flex-1 p-8 lg:p-12 overflow-y-auto">
                ...
                <div className="flex justify-between items-center mb-8">
                    <h1 className="font-['bebas-neue'] font-bold text-4xl text-[#2D2D2D] tracking-wide uppercase">Command Center</h1>
                    <div className="bg-green-50 text-green-600 px-4 py-1.5 rounded-full border border-green-100 text-xs font-bold tracking-widest uppercase flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
                        ...
                        <div style={{ width: '100%', height: 300 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={stats.revenue_trend || []}>
                                    ...
                                    <Bar dataKey="revenue" fill="#6E39CB" radius={[6, 6, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                        ...
                        <div style={{ width: '100%', height: 260, marginTop: '16px' }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={userPieData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={8} dataKey="value" stroke="none">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Metrics Dashboard */}
                    <div className="lg:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-gray-100 min-h-112.5">
                        {orgLoading ? (
                            <div className="h-full flex flex-col items-center justify-center text-gray-400">
                                <div className="w-10 h-10 border-4 border-purple-100 border-t-[#6E39CB] rounded-full animate-spin mb-4"></div>
                                <p className="font-bold text-[10px] tracking-widest uppercase">Fetching Data...</p>
                            </div>
                        ) : !orgStats ? (
                            <div className="h-full flex items-center justify-center text-gray-400 italic">Select an organizer to view their impact.</div>
                        ) : (
                            <div>
                                <div className="grid grid-cols-3 gap-4 mb-8">
                                    <div className="bg-[#F8F7FF] p-6 rounded-2xl border border-purple-50 text-center">
                                        <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-1">Created</p>
                                        <p className="text-4xl font-['bebas-neue'] text-[#6E39CB]">{orgStats.total_events_created}</p>
                                    </div>
                                    <div className="bg-[#F8F7FF] p-6 rounded-2xl border border-purple-50 text-center">
                                        <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-1">Sold</p>
                                        <p className="text-4xl font-['bebas-neue'] text-[#6E39CB]">{orgStats.total_tickets_sold}</p>
                                    </div>
                                    <div className="bg-[#F8F7FF] p-6 rounded-2xl border border-purple-50 text-center">
                                        <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-1">Revenue</p>
                                        <p className="text-4xl font-['bebas-neue'] text-[#6E39CB]">${orgStats.total_revenue}</p>
                                    </div>
                                </div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        <thead className="bg-gray-50 rounded-xl">
                                            <tr>
                                                <th className="py-4 px-6 text-gray-500 text-[10px] font-bold uppercase">Event</th>
                                                <th className="py-4 px-6 text-gray-500 text-[10px] font-bold uppercase">Tickets</th>
                                                <th className="py-4 px-6 text-gray-500 text-[10px] font-bold uppercase text-right">Revenue</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {orgStats.events?.length === 0 ? (
                                                ...
                                                orgStats.events?.map(event => (
                                                    <tr key={event.event_id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors group">
                                                        <td className="py-4 px-6 font-bold text-gray-700 group-hover:text-[#6E39CB]">{event.name}</td>
                                                        <td className="py-4 px-6 text-gray-500">{event.tickets_sold}</td>
                                                        <td className="py-4 px-6 text-green-600 font-black text-right">${event.revenue}</td>
                                                    </tr>
                                                ))
                                            )}
                                        </tbody>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col max-h-137.5">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex justify-between items-center mb-1">
                                                <p className="font-bold text-sm truncate leading-none">{org.name}</p>
                                            ...
                                            <p className={`text-[10px] truncate ${selectedOrgId === org.user_id ? "text-purple-200" : "text-gray-400"}`}>{org.email}</p>
                                        </div>
                                    </div>
                                </button>
                            ))}
