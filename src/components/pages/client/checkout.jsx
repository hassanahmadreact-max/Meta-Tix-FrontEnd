...
function Cart() {
    ...
    const [step, setStep] = useState(1); // 1 = Review Cart, 2 = Payment
    const [loading, setLoading] = useState(false);
    const [reservedBookingIds, setReservedBookingIds] = useState([]);
    ...
    const handleReserve = async () => {
        ...
        try {
            ...
            setReservedBookingIds(bookingIds);
            setStep(2);
        } catch (error) {
            console.error("🚨 Reservation failed:", error);
            alert("Failed to reserve tickets. They might be sold out!");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handlePayment = async () => {
        setLoading(true);
        try {
            for (const bookingId of reservedBookingIds) {
                const payload = {
                    booking_id: bookingId,
                    payment_method: "Credit Card"
                };

                await axios.post("http://127.0.0.1:8000/booking/checkout", payload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            ...
            setCartItems([]);
            alert("🎉 Payment Successful! Your tickets have been generated.");
            navigate("/mytickets"); 
        } catch (error) {
    ...
    return (
        <div className="bg-[#F4F5F9] min-h-screen flex flex-row font-['Lato']">
            ...
            <div className="w-full flex-1 p-4 pt-24 md:p-10 lg:p-16 overflow-y-auto">
                ...
                {/* 🚨 UPDATED DYNAMIC HEADER WITH SVGs */}
                ...
                {cartItems.length === 0 ? (
                    <div className="bg-white p-8 md:p-16 rounded-3xl shadow-sm text-center border border-gray-200 flex flex-col items-center">
                        ...
                        <p className="text-gray-500 mb-8 text-sm md:text-base max-w-md">Looks like you haven't added any tickets yet. Explore our upcoming events and find something you love!</p>
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="flex-1">
                            {step === 1 ? (
                                <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
                                    {cartItems.map((item) => (
                                        <div key={item.cartItemId} className="p-5 md:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between sm:items-center gap-4 sm:gap-0 hover:bg-gray-50 transition-colors">
                                            {/* 🚨 ADDED TICKET SVG NEXT TO EVENT NAME */}
                                ...
                                <div className="bg-white p-5 md:p-8 rounded-3xl shadow-sm border border-gray-200 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    {/* 🚨 UPDATED PAYMENT DETAILS HEADER WITH SVG */}
                                    ...
                                    <div className="space-y-5">
                                        <div>
                                            <label className="block text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Cardholder Name</label>
                                            <input type="text" placeholder="John Doe" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#6E39CB] focus:ring-1 focus:ring-[#6E39CB] transition-all font-medium" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Card Number</label>
                                            <input type="text" placeholder="**** **** **** 1234" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#6E39CB] focus:ring-1 focus:ring-[#6E39CB] transition-all font-medium tracking-widest" />
                                        </div>
                                        <div className="flex gap-4">
                                            <div className="flex-1">
                                                <label className="block text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Expiry Date</label>
                                                <input type="text" placeholder="MM/YY" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#6E39CB] focus:ring-1 focus:ring-[#6E39CB] transition-all font-medium text-center" />
                                            </div>
                                            <div className="flex-1">
                                                <label className="block text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">CVC</label>
                                                <input type="password" placeholder="***" className="w-full p-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#6E39CB] focus:ring-1 focus:ring-[#6E39CB] transition-all font-medium text-center tracking-widest" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="w-full lg:w-96">
                            <div className="bg-white p-5 md:p-8 rounded-3xl shadow-sm border border-gray-200 sticky top-8">
                                {/* 🚨 UPDATED ORDER SUMMARY HEADER WITH SVG */}
                                ...
                                {step === 1 ? (
                                    ...
                                    <button 
                                        ...
                                        disabled={loading}
                                        className="w-full bg-green-600 text-white py-4 rounded-xl font-bold text-base md:text-lg hover:bg-green-700 transition-all disabled:opacity-50 shadow-md flex justify-center items-center gap-3"
                                    >
                                        {loading ? (
